...
public:
	...
	void Create(int _particle_count, float _lifetime, float _size, sf::Vector2f _position);
...
template<typename ParticleType>
inline void ParticleCluster<ParticleType>::Create(int _particle_count, float _lifetime, float _size, sf::Vector2f _position)
{
	particles = std::vector<ParticleType>(_particle_count);

	for (int i = 0; i < particles.size(); i++) {
		particles[i].Create(_lifetime, _size, _position);
	}
}
